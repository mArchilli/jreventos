...
export default function Welcome() {
    return (
        <>
            ...
            {/* INTRO EVENTOS */}
            <section className="bg-black px-8 lg:px-32 pt-24 pb-10 lg:pt-28 lg:pb-10 border-t border-white/5">
                <div className="flex flex-col gap-8">
                    <h2
                        className="font-black text-white leading-none tracking-tight uppercase"
                        style={{ fontSize: 'clamp(32px, 8vw, 115px)' }}
                    >
                        Así realizamos
                        <br />
                        <span className="text-yellow-300">nuestros eventos</span>
                    </h2>
                    <p className="text-white/55 leading-relaxed max-w-3xl" style={{ fontSize: 'clamp(15px, 1.4vw, 19px)' }}>
                        Cada evento nace de una historia distinta y merece una experiencia a su medida. Ponemos creatividad, tecnología y pasión en cada detalle — desde la primera idea hasta el momento que queda grabado en la memoria. Hacé click debajo para conocer todos los eventos que realizamos.
                    </p>
                </div>
            </section>
            ...
            <section className="bg-black py-20 px-8 lg:px-32 overflow-hidden">
                <div className="flex flex-col lg:flex-row items-center gap-16">
                    ...
                    {/* Columnas de logos - DESKTOP (vertical) */}
                    <div className="hidden lg:flex flex-1 gap-8 h-[560px] overflow-hidden">
                        {/* Columna que sube */}
                        <div
                    ...
                    {/* Filas de logos - MOBILE (horizontal) */}
                    <div className="flex lg:hidden flex-col gap-6 w-full overflow-hidden">
                        {/* Fila que va a la izquierda */}
                        <div className="overflow-hidden">
                            <div
                                className="flex animate-scroll-left will-change-transform"
                                style={{ '--set-width': `${clientLogos.length * (160 + 24)}px` }}
                            >
                                {[...clientLogos, ...clientLogos, ...clientLogos].map((src, i) => (
                                    <div key={`left-${i}`} className="flex items-center justify-center bg-white/5 rounded-2xl p-4 shrink-0 mr-6" style={{ width: '160px', height: '120px' }}>
                                        <img src={src} alt="cliente" className="max-h-16 max-w-full object-contain" />
                                    </div>
                                ))}
                            </div>
                        </div>
                        {/* Fila que va a la derecha */}
                        <div className="overflow-hidden">
                            <div
