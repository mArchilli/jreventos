...
export default function EventsShow({ event, relatedEvents = [] }) {
    ...
    const eventWhatsAppHref = getMainWhatsAppHref(`Hola, quiero consultar sobre ${event.title}.`);
    ...
    const [showDesktopFloatingCta, setShowDesktopFloatingCta] = useState(false);
    ...
    useEffect(() => {
        const updateDesktopFloatingCta = () => {
            const heroThreshold = Math.max(window.innerHeight - 160, 320);
            setShowDesktopFloatingCta(window.innerWidth >= 1024 && window.scrollY > heroThreshold);
        };

        updateDesktopFloatingCta();
        window.addEventListener('scroll', updateDesktopFloatingCta, { passive: true });
        window.addEventListener('resize', updateDesktopFloatingCta);

        return () => {
            window.removeEventListener('scroll', updateDesktopFloatingCta);
            window.removeEventListener('resize', updateDesktopFloatingCta);
        };
    }, []);
    ...
    return (
        <>
            ...
            <main className="bg-black text-white">
                        ...
                        <div className="lg:col-span-5 lg:sticky lg:top-28">
                            <div className="rounded-2xl border p-8 flex flex-col gap-6" style={{ background: `linear-gradient(135deg, ${theme.accent}0a 0%, #111 100%)`, borderColor: `${theme.accent}25` }}>
                                ...
                                <div className="flex flex-col gap-3">
                                    <a href={eventWhatsAppHref} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 font-extrabold py-4 px-6 rounded-full text-base hover:scale-[1.02] active:scale-95 transition-all" style={{ background: theme.accent, color: '#000', boxShadow: `0 8px 32px ${theme.accent}30` }}>
                ...
                <section className="px-8 lg:px-32 py-20 border-t border-white/5" style={{ background: 'linear-gradient(180deg, #0a0a0a 0%, #000 100%)' }}>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                        ...
                        <div className="rounded-2xl p-8 lg:p-10 flex flex-col gap-6" style={{ background: `linear-gradient(135deg, ${theme.accent}12 0%, #111 100%)`, border: `1px solid ${theme.accent}30` }}>
                            ...
                            <div className="mt-2">
                                <a href={eventWhatsAppHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 font-extrabold py-4 px-8 rounded-full text-base hover:scale-105 active:scale-95 transition-all" style={{ background: theme.accent, color: '#000', boxShadow: `0 8px 40px ${theme.accent}35` }}>
                ...
                <section className="mx-8 lg:mx-32 mb-24 rounded-2xl overflow-hidden relative" style={{ background: `linear-gradient(135deg, ${theme.accent}18 0%, #111 60%)`, border: `1px solid ${theme.accent}25` }}>
                    ...
                    <div className="relative px-8 lg:px-16 py-12 lg:py-16 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
                        ...
                        <a href={eventWhatsAppHref} target="_blank" rel="noopener noreferrer" className="flex-shrink-0 inline-flex items-center gap-3 font-extrabold py-4 px-9 rounded-full text-base hover:scale-105 active:scale-95 transition-all" style={{ background: theme.accent, color: '#000', boxShadow: `0 8px 40px ${theme.accent}35` }}>
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
            ...
            {/* CTA FIJO DESKTOP */}
            <div
                className="hidden lg:block fixed left-1/2 bottom-8 z-50 transition-all duration-300"
                style={{
                    opacity: showDesktopFloatingCta ? 1 : 0,
                    transform: `translateX(-50%) translateY(${showDesktopFloatingCta ? '0' : '18px'})`,
                    pointerEvents: showDesktopFloatingCta ? 'auto' : 'none',
                }}
            >
                <a
                    ...
                    className="inline-flex items-center justify-center gap-3 rounded-full px-7 py-4 text-sm font-extrabold uppercase tracking-[0.12em] text-black shadow-[0_16px_40px_rgba(0,0,0,0.35)] transition-transform duration-200 hover:scale-105 active:scale-95"
            ...
            <div className="fixed bottom-0 left-0 right-0 lg:hidden z-50 p-4 bg-gradient-to-t from-black via-black/90 to-transparent">
                <a href={eventWhatsAppHref} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 w-full font-extrabold py-4 px-6 rounded-full text-base active:scale-95 transition-all" style={{ background: theme.accent, color: '#000', boxShadow: `0 8px 40px ${theme.accent}50` }}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                    {theme.ctaLabel}
                </a>
            </div>
